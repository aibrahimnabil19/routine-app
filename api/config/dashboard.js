...
const EMOJIS = ['📖','💧','🧘','🏃','🧹','💊','✍️','🎨','🎯','🧠','🍎','🛌','🚿','🎵','☎️','💼'];
const TZ_LIST = [
  'UTC','Africa/Lagos','Africa/Niamey','Africa/Cairo','Africa/Nairobi','Africa/Johannesburg',
  ...
  'Asia/Tokyo','Asia/Jakarta','Australia/Sydney'
];
...
function renderTimezoneOptions() {
  ...
  sel.innerHTML = TZ_LIST.map(tz => `<option value="${tz}">${tz}</option>`).join('');
}
...
async function loadTasks() {
  ...
  if (error) { console.error(error); listEl.innerHTML = '<div class="empty-state">Could not load tasks.</div>'; return; }
...
function renderTasks() {
  ...
  tasks.forEach(t => {
    ...
    const subtitle = t.type === 'alternating'
      ? (t.activeItem ? `Now: ${t.activeItem.label} · Streak ${t.current_streak}d` : `All items done · Streak ${t.current_streak}d`)
...
async function toggleComplete(taskId) {
  ...
  if (t.completedToday) {
    // undo
    await supabase.from('task_completions').delete().eq('task_id', taskId).eq('log_date', today);
    const newStreak = Math.max(0, (t.current_streak || 1) - 1);
    await supabase.from('tasks').update({ current_streak: newStreak }).eq('id', taskId);
    await loadTasks();
    return;
  }
  ...
  const { error: insErr } = await supabase.from('task_completions').insert({
    task_id: taskId, user_id: user.id, log_date: today, item_label: itemLabel
  });
  if (insErr) { toast('Could not save — try again.'); return; }

  await supabase.from('tasks').update({
    current_streak: newStreak, longest_streak: longest, last_completed_date: today
  }).eq('id', taskId);

  // advance alternating queue
  if (t.type === 'alternating' && t.activeItem) {
    await supabase.from('task_items').update({ status: 'done' }).eq('id', t.activeItem.id);
    const { data: nextItem } = await supabase
      ...
      .maybeSingle();
    if (nextItem) {
      await supabase.from('task_items').update({ status: 'active' }).eq('id', nextItem.id);
    } else {
      toast(`You finished your list for "${t.title}" 🎉 Add more items any time.`);
    }
  }
...
function openTaskModal(task = null) {
  ...
  $('taskReminder').value = task ? (task.reminder_time || '08:00').slice(0,5) : '08:00';
  setType(currentType);
...
async function loadItemsForEdit(taskId) {
  const { data } = await supabase.from('task_items').select('*').eq('task_id', taskId).order('position');
...
function setType(type) {
  ...
  $('altItemsSection').style.display = type === 'alternating' ? 'block' : 'none';
  $('titleLabel').textContent = type === 'alternating' ? 'Goal name (e.g. Reading list)' : 'Title';
  if (type === 'alternating' && $('itemsList').children.length === 0) addItemRow('');
}
...
async function saveTask() {
  ...
  const payload = {
    title, icon, duration_minutes: duration, reminder_time: reminderTime,
    type: currentType, user_id: user.id, is_active: true
  };
  ...
  if (editingTaskId) {
    ...
    if (error) { toast('Could not save goal.'); return; }
    taskId = data.id;
  }
  ...
  if (currentType === 'alternating') {
    ...
    if (editingTaskId) {
      ...
      const { data: existing } = await supabase.from('task_items').select('label').eq('task_id', taskId);
...
async function insertItems(taskId, labels, startPosition) {
  ...
  const { data: activeExisting } = await supabase.from('task_items')
    .select('id').eq('task_id', taskId).eq('status', 'active').maybeSingle();

  const rows = labels.map((label, idx) => ({
    task_id: taskId, user_id: user.id, label,
...
async function saveSettings() {
  ...
  await supabase.from('profiles').update({ full_name, timezone, reminders_enabled }).eq('id', user.id);
...
function bindEvents() {
  ...
  $('closeSettingsBtn').addEventListener('click', () => $('settingsModalOverlay').style.display = 'none');
  ...
  $('taskList').addEventListener('click', (e) => {
    ...
    if (dot) { toggleComplete(dot.dataset.id); return; }
